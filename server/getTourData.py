...
class KomootSocial:
        ...
        try:
            ...
            for tours in ListOfTours:
                ...
                try:
                    ...
                    val = (TourDate, TourDistance, TourDuration, TourElevation_up, TourElevation_down,
                           TourMap_image, TourName, TourSport, TourStart_point, TourType, TourID, TourCreatorID)
    ...
    def getHowManyToursInMonthPerYear(self, tour_year):
        ...
        mycursor = self.mydb.cursor()
        sql = f"SELECT DATE_FORMAT(tour_date , '%M'), COUNT(*) FROM _{self.client_id} WHERE tour_date LIKE '{tour_year}%' GROUP BY DATE_FORMAT(tour_date , '%M')"
        mycursor.execute(sql)
        rows = mycursor.fetchall()
        for i in range(len(rows)):
            months.append(rows[i][0])
            i = i+1
        for i in range(len(rows)):
            times.append(rows[i][1])
            i = i+1

        objects_list.append(dict(zip(months, times)))
...
if (functionType == "toursInMonthPerYear"):
    ks = KomootSocial(int(sys.argv[2]), sys.argv[3], sys.argv[4])
    ks.getTourData()
